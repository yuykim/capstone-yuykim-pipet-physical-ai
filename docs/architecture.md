...
| `camera_setup` | () | 카메라 구성 metadata, 학습 변환 제외 |
...
| `gripper_actions` | (N,) | 그리퍼 모드: 0=유지, 1=잡기, 2=펴기, 3=누르기, 4=엄지 펴기 |
...
- 초기 목표: 50 에피소드
- 이후 결과를 보며 점진적으로 증가
- 현재 실제 수집 태스크의 성공 기준은 파이펫을 잡아서 들어 올리는 것까지로 둔다.

#### 유의사항
- 쓰레기 데이터보다 깔끔한 시연이 중요 → 에피소드 검수 기준 필요
- 거치대 위치/조명을 조금씩 다르게 수집하되, 파이펫 거치대는 `docs/data_collect_env/README.md`의 노란색 배치 가능 영역 밖으로 벗어나지 않게 한다.
- 파이펫 각도는 기본적으로 고정하고, 사람이 조작하며 생기는 작은 오차만 허용한다.
