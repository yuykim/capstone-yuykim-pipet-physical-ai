...
| 종류 | 이름 | 타입 | 설명 |
|------|------|------|------|
| 토픽 (발행) | `/gripper/status` | (자체 정의) | 그리퍼 현재 상태, 20Hz |
| 서비스 | `/gripper/grasp` | std_srvs/Trigger | 파이펫 잡기 |
| 서비스 | `/gripper/open` | std_srvs/Trigger | 손 펴기 |
| 서비스 | `/gripper/press` | std_srvs/Trigger | 파이펫 누르기 |

#### RealSense 모듈 (`ros-humble-realsense2-camera`, 공식 패키지)

| 종류 | 이름 | 타입 | 설명 |
|------|------|------|------|
| 토픽 (발행) | `/camera/camera/color/image_raw` | sensor_msgs/Image | RGB 이미지, 15~30Hz |
| 토픽 (발행) | `/camera/camera/aligned_depth_to_color/image_raw` | sensor_msgs/Image | Depth 이미지, 15~30Hz |
...
| 장치 | 연결 방식 | 인터페이스 |
|------|----------|------------|
| Indy7 | 이더넷 (기본) / WiFi (대안) | TCP/IP — 고정 IP `192.168.1.100` |
