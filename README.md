...
→ 키보드로 EE pose / joint 제어 (`movetelel_abs` / `movetelej_abs`).
...
Neuromeka 표준 키 + 그리퍼 (G/O/B/V).

| 키 | 동작 |
|---|---|
| ↑ ↓ ← → | x +/− , y +/− (mm) |
| `.` / `;` | z +/− (mm) |
| N / M / `,` | rx / ry / rz (방향은 R 토글) |
| 1 ~ 7 | 조인트 i 조그 (방향은 R 토글) |
| W / E | base / tool 좌표계 |
| R | 조그 방향 토글 (+ ↔ −) |
| `−` / `+` | 조인트 step 증감 |
| 9 / 0 | Cartesian step 증감 |
| H / Z / S / P | Home / Zero / Recover / Stop teleop |
| **G / O / B / V** | Grasp / Open / Press / Release |
| Q | 종료 |
...
1. 터미널 3에서 `keyboard_servo_node` 실행
2. 터미널 2에서 **`2`** → 키보드 모드
3. **`SPACE`** → 녹화 시작
4. 터미널 3에서 키보드로 조작 + 그리퍼
5. 터미널 2에서 **`SPACE`** → 라벨링
