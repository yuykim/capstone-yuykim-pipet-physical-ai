#!/usr/bin/env bash
# 26.04.07 LeRobotDataset으로 ACT 학습: 10만 스텝, 1만 스텝마다 체크포인트 저장.
# 체크포인트: <repo>/ai/models/26.04.07/checkpoints/<step>/
#
...
#         rm -rf "${REPO_ROOT}/ai/models/26.04.07"
...
OUT="${REPO_ROOT}/ai/models/26.04.07"

python3 "${REPO_ROOT}/ai/lerobot/run_lerobot_train.py" \
  --skip_convert \
  --episodes_dir "${REPO_ROOT}/ros2_ws/episodes/success/26.04.07" \
  --dataset_output_dir "${REPO_ROOT}/ai/datasets/26.04.07" \
  --dataset_repo_id pipet_26_04_07 \
  --output_dir "${OUT}" \
  --job_name act_26_04_07 \
  --steps 100000 \
  --eval_freq 10000 \
  --log_freq 50 \
  --save_freq 10000 \
